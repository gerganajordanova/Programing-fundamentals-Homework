...
namespace _02.Command_Interpreter
{
    public class CommandInterpreter
    {
        ...
        {
            List<string> sequence = Console.ReadLine().Trim().Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries).ToList();
                    ...
                    case "reverse":
                        {
                            ...
                            if (isValid(start, count, sequence))
                            {
                                sequence.Reverse(start, count);
                            }
                            else
                            {
                                Console.WriteLine("Invalid sequence parameters.");
                            }
                            break;
                        }
                    case "sort":
                        {
                            ...
                            if (isValid(start, count, sequence))
                            {
                                sequence.Sort(start, count, StringComparer.InvariantCulture);
                            }
                            else
                            {
                                Console.WriteLine("Invalid sequence parameters.");
